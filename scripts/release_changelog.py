...
practical area, and keeps commit links so the detail is traceable.
"""
...
CATEGORIES = (
    Category(
        "Music dashboard UI and settings",
        paths=(
            "common/addon/",
            "common/device/",
            "devices/",
    ...
    Category(
        ...
        paths=("builds/", "common/", "components/", "devices/"),
        keywords=("compile", "device", "firmware", "ota", "wifi", "esp32", "p4", "s3"),
    ),
    Category(
        ...
        keywords=("asset", "changelog", "manifest", "release", "tag", "update", "version"),
    ),
    Category(
    ...
    Category(
        ...
        keywords=("doc", "docs", "readme", "troubleshooting"),
    ),
    Category(
        "Build, tests, and maintenance",
        paths=(".agents/", ".github/", "package.json", "package-lock.json", "renovate.json", "scripts/"),
        keywords=("ci", "check", "deps", "maintenance", "refactor", "test"),
    ),
)
...
def validate_range(from_ref: str | None, to_ref: str) -> None:
    if from_ref and not ref_exists(from_ref):
        raise ChangelogError(f"Release range start does not exist: {from_ref}")
    if not ref_exists(to_ref):
        raise ChangelogError(f"Release range end does not exist: {to_ref}")
    if from_ref and not is_ancestor(from_ref, to_ref):
        raise ChangelogError(f"Release range start {from_ref} is not reachable from {to_ref}")
...
def load_commits(from_ref: str | None, to_ref: str) -> list[Commit]:
    validate_range(from_ref, to_ref)
    output = run_git(
        [
            "log",
            "--reverse",
...
def build_changelog(version: str, from_ref: str | None, to_ref: str, repo_url: str | None) -> str:
    ...
    if not commits:
        lines.extend(["No commits were found in this release range.", ""])
        return "\n".join(lines)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("version", help="Release version to show in the changelog, for example v2.0.3")
    parser.add_argument("--from", dest="from_ref", help="Previous release tag or commit. Defaults to previous stable tag.")
